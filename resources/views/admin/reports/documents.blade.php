        ...
        <div class="rounded-lg overflow-hidden shadow border print:shadow-none print:border-0" style="background-color: white; border-color: #E0E0E0;">
            <div class="px-6 py-4 border-b print:bg-white" style="background-color: #F8F9FA; border-color: #E0E0E0;">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
                    <h2 class="text-lg font-semibold" style="color: #212121; font-size: 18px;">Document List</h2>
                ...
                <div class="px-6 py-4 border-b" style="border-color: #E0E0E0;">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        <div class="p-4 rounded-lg" style="background-color: #E8F5E8;">
                            <div class="text-sm" style="color: #424242;">Total Documents</div>
                            <div class="text-xl font-bold" style="color: #2E7D32;">{{ count($data) }}</div>
                        </div>
                        <div class="p-4 rounded-lg" style="background-color: #E8F5E8;">
                            <div class="text-sm" style="color: #424242;">Verified</div>
                            <div class="text-xl font-bold" style="color: #2E7D32;">{{ $data->where('status', 'Verified')->count() }}</div>
                        </div>
                        ...
                        <div class="p-4 rounded-lg" style="background-color: #FFEBEE;">
                            <div class="text-sm" style="color: #424242;">Rejected</div>
                            <div class="text-xl font-bold" style="color: #D32F2F;">{{ $data->where('status', 'Rejected')->count() }}</div>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="min-w-full divide-y" style="border-color: #E0E0E0;">
                            <thead class="print:bg-white" style="background-color: #F8F9FA;">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider" style="color: #757575;">Type</th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider" style="color: #757575;">Status</th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider" style="color: #757575;">Uploaded</th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider" style="color: #757575;">Verified</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y" style="background-color: white; border-color: #E0E0E0;">
                                @foreach($data as $document)
                                    <tr style="transition: background-color 0.15s ease;"
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm" style="color: #424242;">{{ $document->title }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                ...
                                                <div class="ml-3 print:ml-0">
                                                    <div class="text-sm font-medium" style="color: #212121;">{{ $document->user->name ?? 'Unknown' }}</div>
                                                    <div class="text-xs" style="color: #757575;">{{ $document->user->email }}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm" style="color: #424242;">{{ ucfirst($document->document_type) }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-2 py-1 text-xs rounded-full
                                                @if($document->status == 'Verified') " style="background-color: #E8F5E8; color: #1B5E20;"
                                                @elseif($document->status == 'Rejected') " style="background-color: #FFEBEE; color: #B71C1C;"
                                                @else " style="background-color: #FFF3C4; color: #F57F17;" @endif">
                                                {{ $document->status }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm" style="color: #424242;">{{ $document->created_at->format('M d, Y') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm" style="color: #424242;">{{ $document->verified_at ? date('M d, Y', strtotime($document->verified_at)) : 'N/A' }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                ...
                @else
                    <div class="text-center py-12">
                        <div class="w-16 h-16 mx-auto rounded-full flex items-center justify-center mb-4" style="background-color: #F8F9FA;">
                            <i class="fas fa-file-alt text-2xl" style="color: #757575;"></i>
                        </div>
                        <h3 class="text-lg font-medium mb-2" style="color: #212121;">No Documents Found</h3>
                        <p class="mb-6" style="color: #757575;">There are no documents matching your criteria.</p>
                    </div>
                @endif
            </div>
        </div>
