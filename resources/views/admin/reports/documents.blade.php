        ...
        <div class="print:mt-8">
            ...
            <div class="px-6 py-4 border-b border-gray-200">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div class="bg-blue-50 p-4 rounded-lg">
                        ...
                        <div class="text-xl font-bold text-gray-800">{{ $data->where('status', 'Verified')->count() }}</div>
                    </div>
                    ...
                    <div class="bg-red-50 p-4 rounded-lg">
                        ...
                        <div class="text-xl font-bold text-gray-800">{{ $data->where('status', 'Rejected')->count() }}</div>
                    </div>
                </div>
            </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($data as $document)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $document->title }}</td>
                                        ...
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-8 w-8 rounded-full bg-gray-200 flex items-center justify-center print:hidden">
                                                @if($document->user && $document->user->profile_photo)
                                                    <img src="{{ asset('storage/' . $document->user->profile_photo) }}" alt="{{ $document->user->name }}" class="h-8 w-8 rounded-full">
                                                @else
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ ucfirst($document->document_type) }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $document->created_at->format('M d, Y') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $document->verified_at ? date('M d, Y', strtotime($document->verified_at)) : 'N/A' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-500 mb-6">There are no documents matching your criteria.</p>
                </div>
            @endif
        </div>
    </div>
