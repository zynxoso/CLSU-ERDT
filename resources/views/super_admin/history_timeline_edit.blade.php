...
@section('title', 'Edit History Timeline Item')
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit History Timeline Item</h1>
            <p class="text-gray-600">Update timeline item for the history page</p>
        </div>
    ...
    <div class="bg-white shadow-sm rounded-lg">
        ...
        <form action="{{ route('super_admin.history_timeline.update', $timelineItem->id) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text"
                           name="title"
                           id="title"
                           value="{{ old('title', $timelineItem->title) }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description"
                              id="description"
                              rows="4"
                              class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $timelineItem->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date"
                           name="event_date"
                           id="event_date"
                           value="{{ old('event_date', $timelineItem->event_date->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <input type="text"
                           name="year_label"
                           id="year_label"
                           value="{{ old('year_label', $timelineItem->year_label) }}"
                ...
                <div>
                    ...
                    <select name="category"
                        ...
                        <option value="milestone" {{ old('category', $timelineItem->category) == 'milestone' ? 'selected' : '' }}>Milestone</option>
                        <option value="achievement" {{ old('category', $timelineItem->category) == 'achievement' ? 'selected' : '' }}>Achievement</option>
                        <option value="partnership" {{ old('category', $timelineItem->category) == 'partnership' ? 'selected' : '' }}>Partnership</option>
                    </select>
                    @error('category')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="color"
                        ...
                        <option value="green" {{ old('color', $timelineItem->color) == 'green' ? 'selected' : '' }}>Green</option>
                        <option value="blue" {{ old('color', $timelineItem->color) == 'blue' ? 'selected' : '' }}>Blue</option>
                        <option value="purple" {{ old('color', $timelineItem->color) == 'purple' ? 'selected' : '' }}>Purple</option>
                        <option value="red" {{ old('color', $timelineItem->color) == 'red' ? 'selected' : '' }}>Red</option>
                        <option value="yellow" {{ old('color', $timelineItem->color) == 'yellow' ? 'selected' : '' }}>Yellow</option>
                        <option value="indigo" {{ old('color', $timelineItem->color) == 'indigo' ? 'selected' : '' }}>Indigo</option>
                    </select>
                    @error('color')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text"
                           name="icon"
                           id="icon"
                           value="{{ old('icon', $timelineItem->icon) }}"
                ...
                <div>
                    ...
                    <input type="number"
                           name="sort_order"
                           id="sort_order"
                           value="{{ old('sort_order', $timelineItem->sort_order) }}"
                           min="0"
                ...
                @if($timelineItem->image_path)
                    <div class="md:col-span-2">
                        ...
                        <div class="flex items-center space-x-4">
                            <img src="{{ Storage::url($timelineItem->image_path) }}"
                                 alt="Current image"
                                 class="h-20 w-20 object-cover rounded-lg border border-gray-300">
                            <div>
                                <p class="text-sm text-gray-600">Current image uploaded</p>
                ...
                <div class="md:col-span-2">
                    <label for="image" class="block text-sm font-medium text-gray-700 mb-2">
                        {{ $timelineItem->image_path ? 'Replace Image (Optional)' : 'Image (Optional)' }}
                    </label>
                    <input type="file"
                           name="image"
                           id="image"
