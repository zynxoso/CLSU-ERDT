    ...
    <form action="{{ route('scholar.manuscripts.update', $manuscript->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <textarea id="abstract" name="abstract" rows="4"
                class="w-full bg-white border @error('abstract') border-red-300 @else border-gray-300 @enderror rounded-md px-3 py-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500" required>{{ old('abstract', $manuscript->abstract) }}</textarea>
            @error('abstract')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        ...
        <div class="flex justify-between items-center">
            ...
            <div class="flex justify-end space-x-3">
                <a href="{{ route('scholar.manuscripts.show', $manuscript->id) }}" class="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700">
                    Cancel
                </a>
