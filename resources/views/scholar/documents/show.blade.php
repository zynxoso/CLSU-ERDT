            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                <div class="flex items-center mb-4 md:mb-0">
                    <div class="w-12 h-12 rounded-full bg-blue-500 flex items-center justify-center mr-4">
                        <i class="fas fa-file" style="color: rgb(255 255 255) !important;"></i>
                    </div>
                    <div>
                ...
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('scholar.documents.download', $document->id) }}" class="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors duration-300">
                        <i class="fas fa-download mr-2" style="color: rgb(255 255 255) !important;"></i> Download Document
                    </a>
                </div>
            </div>
        </div>
            ...
            <div class="p-6">
                @if(in_array(pathinfo($document->file_name, PATHINFO_EXTENSION), ['pdf']))
                    <div class="bg-gray-100 p-4 rounded-lg">
                        <embed src="{{ route('scholar.documents.view', $document->id) }}" type="application/pdf" width="100%" height="600px" />
                    </div>
                @elseif(in_array(pathinfo($document->file_name, PATHINFO_EXTENSION), ['jpg', 'jpeg', 'png']))
                    <div class="bg-gray-100 p-4 rounded-lg text-center">
                        <img src="{{ route('scholar.documents.view', $document->id) }}" alt="{{ $document->title }}" class="max-w-full h-auto mx-auto" style="max-height: 600px;">
                    </div>
                @else
                    <div class="bg-gray-100 p-8 rounded-lg text-center">
                        ...
                        <a href="{{ route('scholar.documents.download', $document->id) }}" class="mt-4 inline-block px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors duration-300">
                            <i class="fas fa-download mr-2" style="color: rgb(255 255 255) !important;"></i> Download to View
                        </a>
                    </div>
                @endif
            </div>
