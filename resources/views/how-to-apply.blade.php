...
<div class="py-12 sm:py-16 lg:py-24 bg-white" id="downloadable-forms">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="max-w-7xl mx-auto">
            @php
                $allForms = collect();
                if(isset($downloadableForms)) {
                    foreach($downloadableForms as $category => $forms) {
                        $allForms = $allForms->merge($forms);
                    }
                }
            @endphp
            ...
            <!-- Download All Card -->
            @if($allForms->count() > 0)
                <div class="bg-gradient-to-r from-green-600 to-green-700 rounded-2xl p-6 mb-8 shadow-lg">
                    <div class="flex flex-col sm:flex-row items-center justify-between">
                        <div class="text-white mb-4 sm:mb-0">
                            <h3 class="text-2xl text-white font-bold mb-2">Download All Forms</h3>
                            <p class="text-green-100">Get all {{ $allForms->count() }} forms in one convenient package</p>
                        </div>
                        <div class="flex flex-col sm:flex-row gap-3">
                            <div class="text-white text-center sm:text-right">
                                <div class="text-3xl font-bold">{{ $allForms->count() }}</div>
                                <div class="text-green-200 text-sm">Total Forms</div>
                            </div>
                            <button class="bg-white text-green-600 hover:bg-green-50 font-semibold py-3 px-6 rounded-xl transition-all duration-300 shadow-md hover:shadow-lg flex items-center justify-center min-w-[160px]">
            ...
            @if($allForms->count() > 0)
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                    <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
                        <h3 class="text-xl font-semibold text-gray-800">Available Forms</h3>
                        <p class="text-gray-600 mt-1">Click to download individual forms</p>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            <thead class="bg-gray-50 border-b border-gray-200">
                                 <tr>
                                     <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Form Details</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($allForms as $form)
                                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                                         <td class="px-6 py-4">
                                             <div class="flex items-start">
                                                 <div class="flex-shrink-0 w-10 h-10 bg-green-100 text-green-600 rounded-lg flex items-center justify-center mr-4">
                                                     <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                         <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                                     </svg>
                                                 </div>
                                                 <div class="flex-1">
                                                     <h4 class="text-lg font-semibold text-gray-900 mb-1">{{ $form->title }}</h4>
                                                     <p class="text-sm text-gray-600 leading-relaxed mb-2">{{ Str::limit($form->description, 120) }}</p>
                                                     <div class="flex items-center gap-4 text-xs text-gray-500">
                                                         <span class="flex items-center">
                                                             <svg class="w-3 h-3 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                                             </svg>
                                                             {{ strtoupper($form->getFileExtension()) }} • {{ $form->getFileSize() }}
                                                         </span>
                                                         <span class="flex items-center">
                                                             <svg class="w-3 h-3 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3" />
                                                             </svg>
                                                             {{ number_format($form->download_count) }} downloads
                                                         </span>
                                                     </div>
                                                 </div>
                                             </div>
                                         ...
                                         <td class="px-6 py-4 text-right">
                                             <a href="{{ route('downloadable-forms.download', $form) }}" class="inline-flex items-center bg-green-600 text-white hover:bg-green-700 focus:outline-none focus:ring-4 focus:ring-green-300 font-medium py-2 px-4 rounded-lg transition-all duration-300 text-sm">
            ...
            @else
                <div class="bg-white rounded-2xl shadow-lg p-12 text-center">
                    ...
                    <h3 class="text-xl font-semibold text-gray-800 mb-2">No Forms Available</h3>
                    <p class="text-gray-500">There are currently no downloadable forms available. Please check back later.</p>
                </div>
            @endif
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() { const cardViewBtn = document.getElementById('card-view-btn'); const tableViewBtn = document.getElementById('table-view-btn'); const cardView = document.querySelector('.timeline-cards'); const tableView = document.querySelector('.timeline-table'); const viewPreference = localStorage.getItem('timelineViewPreference') || 'card'; if (viewPreference === 'card') { if (cardView) cardView.classList.remove('hidden'); if (tableView) tableView.classList.add('hidden'); if (cardViewBtn) { cardViewBtn.classList.add('bg-green-100'); cardViewBtn.setAttribute('aria-pressed', 'true'); } if (tableViewBtn) { tableViewBtn.setAttribute('aria-pressed', 'false'); } } else { if (tableView) tableView.classList.remove('hidden'); if (cardView) cardView.classList.add('hidden'); if (tableViewBtn) { tableViewBtn.classList.add('bg-green-100'); tableViewBtn.setAttribute('aria-pressed', 'true'); } if (cardViewBtn) { cardViewBtn.setAttribute('aria-pressed', 'false'); } } if (cardViewBtn) { cardViewBtn.addEventListener('click', function() { if (cardView) { cardView.classList.remove('hidden'); cardView.classList.add('fade-in-up'); setTimeout(() => { cardView.classList.remove('fade-in-up'); }, 600); } if (tableView) tableView.classList.add('hidden'); cardViewBtn.classList.add('bg-green-100'); cardViewBtn.setAttribute('aria-pressed', 'true'); if (tableViewBtn) { tableViewBtn.classList.remove('bg-green-100'); tableViewBtn.setAttribute('aria-pressed', 'false'); } localStorage.setItem('timelineViewPreference', 'card'); }); } if (tableViewBtn) { tableViewBtn.addEventListener('click', function() { if (tableView) { tableView.classList.remove('hidden'); tableView.classList.add('fade-in-up'); setTimeout(() => { tableView.classList.remove('fade-in-up'); }, 600); } if (cardView) cardView.classList.add('hidden'); tableViewBtn.classList.add('bg-green-100'); tableViewBtn.setAttribute('aria-pressed', 'true'); if (cardViewBtn) { cardViewBtn.classList.remove('bg-green-100'); cardViewBtn.setAttribute('aria-pressed', 'false'); } localStorage.setItem('timelineViewPreference', 'table'); }); } }); const mobileMenuButton = document.getElementById('mobile-menu-button'); const mobileMenu = document.getElementById('mobile-menu'); mobileMenuButton.addEventListener('click', () => { mobileMenu.classList.toggle('hidden'); }); window.addEventListener('scroll', () => { const winScroll = document.body.scrollTop || document.documentElement.scrollTop; const height = document.documentElement.scrollHeight - document.documentElement.clientHeight; const scrolled = (winScroll / height) * 100; const progressBar = document.getElementById('reading-progress'); if (progressBar) { progressBar.style.width = scrolled + '%'; } }); document.querySelectorAll('a[href^="#"]').forEach(anchor => { anchor.addEventListener('click', function (e) { e.preventDefault(); const target = document.querySelector(this.getAttribute('href')); if (target) { target.scrollIntoView({ behavior: 'smooth', block: 'start' }); } }); }); const observerOptions = { threshold: 0.1, rootMargin: '0px 0px -50px 0px' }; const observer = new IntersectionObserver((entries) => { entries.forEach(entry => { if (entry.isIntersecting) { entry.target.classList.add('fade-in-up'); } }); }, observerOptions); document.querySelectorAll('.transform').forEach(el => { observer.observe(el); }); function downloadAllForms() { if (window.markDownloadStart) { window.markDownloadStart(); } const forms = [ '{{ asset("forms/ERDT_Application_Form.pdf") }}', '{{ asset("forms/Deed-of-Undertaking-form.pdf") }}', '{{ asset("forms/ERDT-SP-form-2024.pdf") }}', '{{ asset("forms/ERDT-TDG-form.pdf") }}', '{{ asset("forms/ERDT-RG-Form-with-sample-LIB.pdf") }}', '{{ asset("forms/ERDT-RDG-form.pdf") }}', '{{ asset("forms/ERDT-LIQUIDATION-REPORT-FOR-OUTRIGHT-THESIS-GRANT.docx") }}', '{{ asset("forms/ERDT-Monitoring-FORM.docx") }}', '{{ asset("forms/Form_SFA-for-scholar.xlsx") }}' ]; const button = event.target; const originalText = button.innerHTML; button.innerHTML = '<svg class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>Downloading...'; button.disabled = true; forms.forEach((formUrl, index) => { setTimeout(() => { const link = document.createElement('a'); link.href = formUrl; link.download = ''; document.body.appendChild(link); link.click(); document.body.removeChild(link); }, index * 500); }); setTimeout(() => { button.innerHTML = originalText; button.disabled = false; if (window.markDownloadEnd) { window.markDownloadEnd(); } }, forms.length * 500 + 1000); }</script>
