        ...
        <div class="bg-white p-4 rounded-lg shadow">
            <form id="filter-form" class="flex flex-wrap gap-4">
                <div class="flex-1 min-w-[200px]">
                    ...
                    <select id="status" name="status" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-600 focus:border-red-600">
                ...
                {{-- <div class="flex-1 min-w-[200px]">
                    ...
                    <select id="purpose" name="purpose" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-600 focus:border-red-600">
                ...
                <div class="flex-1 min-w-[200px]">
                    ...
                    <input type="text" id="scholar" name="scholar" value="{{ request('scholar') }}" placeholder="Scholar Name" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-600 focus:border-red-600">
                </div>
                <div class="flex-1 min-w-[200px]">
                    <label for="date" class="block text-sm font-medium text-gray-700 mb-1">Date Range</label>
                    <input type="month" id="date" name="date" value="{{ request('date') }}" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-600 focus:border-red-600">
                </div>
                <div class="flex items-end w-full sm:w-auto">
                    <button type="button" id="filter-button" class="w-full sm:w-auto px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors duration-150">
    ...
    <div id="document-preview-modal" class="fixed inset-0 z-50 overflow-auto bg-black bg-opacity-50 flex items-center justify-center hidden">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
            <div class="bg-gray-100 px-6 py-4 border-b border-gray-200 flex justify-between items-center sticky top-0 z-10">
                ...
                <button type="button" onclick="closeDocumentModal()" class="text-red-600 hover:text-red-800 transition-colors duration-150">
...
<script>
    ...
    function openDocumentModal(requestId) {
        ...
        // Show loading indicator
        container.innerHTML = '<div class="flex justify-center items-center h-32"><div class="animate-spin rounded-full h-10 w-10 border-b-2 border-red-600"></div></div>';
        ...
        fetch(`/admin/fund-requests/${requestId}/documents`)
            ...
            .then(data => {
                ...
                if (data.documents && data.documents.length > 0) {
                    let documentsHtml = '<div class="grid gap-4">';
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-500">${doc.file_type || 'Document'}</p>
                                        </div>
                                    </div>
            ...
            .catch(error => {
                ...
                container.innerHTML = `
                    <div class="text-center py-8">
                        ...
                        <p class="text-gray-500">There was an error loading the documents. Please try again.</p>
                    </div>
                `;
            });
    }
    ...
    document.getElementById('document-preview-modal').addEventListener('click', function(e) {
        if (e.target === this) {
            closeDocumentModal();
        }
    });
