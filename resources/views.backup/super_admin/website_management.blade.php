                    ...
                    <template x-for="faculty in facultyMembers" :key="faculty.id">
                        <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                            <div class="text-center">
                                <div class="mb-4">
                                    <img x-show="faculty.photo_path" :src="`{{ asset('storage/') }}/${faculty.photo_path}`" :alt="faculty.name"
                                         class="w-20 h-20 rounded-full mx-auto object-cover">
                                    <div x-show="!faculty.photo_path" class="w-20 h-20 rounded-full mx-auto bg-gray-300 flex items-center justify-center">
                                ...
                                <p class="text-xs text-gray-600 mb-1" x-text="faculty.department"></p>
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" id="faculty_department" x-model="facultyForm.department"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                        ...
                        <div>
                            ...
                            <textarea id="faculty_education" x-model="facultyForm.education_background" rows="3"
                                      class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
...
function websiteManagement() {
    return {
        ...
        // Faculty Data - Load from backend
        facultyMembers: @json($facultyMembers ?? []),
        ...
        editFaculty(faculty) {
            ...
            this.facultyForm = {
                ...
                education_background: faculty.education_background || '',
                degree_level: faculty.degree_level || 'Ph.D',
                university_origin: faculty.university_origin || '',
                expertise_tags: faculty.expertise_tags || [],
        ...
        async saveFaculty() {
            try {
                ...
                // Add all form fields
                Object.keys(this.facultyForm).forEach(key => {
                    if (key === 'expertise_tags' && Array.isArray(this.facultyForm[key])) {
                        this.facultyForm[key].forEach((tag, index) => {
                            formData.append(`expertise_tags[${index}]`, tag);
                        });
                    } else if (this.facultyForm[key] !== null) {
                        formData.append(key, this.facultyForm[key]);
                    }
                });
                ...
                const url = this.editingFaculty
                    ? `{{ route('super_admin.faculty.update', '') }}/${this.editingFaculty.id}`
                    : '{{ route("super_admin.faculty.store") }}';

                const method = this.editingFaculty ? 'PUT' : 'POST';
                ...
                if (data.success) {
                    if (this.editingFaculty) {
                        ...
                        this.facultyMembers[index] = data.faculty;
                    } else {
                        // Add new faculty
                        this.facultyMembers.push(data.faculty);
                    }
        ...
        async deleteFaculty(id) {
            if (confirm('Are you sure you want to delete this faculty member?')) {
                try {
                    const response = await fetch(`{{ route('super_admin.faculty.delete', '') }}/${id}`, {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
