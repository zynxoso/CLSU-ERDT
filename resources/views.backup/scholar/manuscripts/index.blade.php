...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="{{ route('scholar.manuscripts.create') }}" class="px-4 py-2 bg-red-800 text-white rounded-lg hover:bg-red-900 transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 py-1 text-xs rounded-full inline-flex items-center
                                    ...
                                    @elseif($manuscript->status == 'Submitted') bg-red-100 text-red-700
                                    ...
                                    @elseif($manuscript->status == 'Submitted')
                                        <i class="fas fa-file-alt mr-1" style="color: #7f1d1d;"></i>
                                    @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="{{ route('scholar.manuscripts.show', $manuscript->id) }}" class="text-blue-700 hover:text-blue-900 transition-colors duration-200 hover:underline">
                                     view manuscript
                                </a>
                                ...
                                <form id="submitForm{{ $manuscript->id }}" action="{{ route('scholar.manuscripts.submit', $manuscript->id) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="button" onclick="confirmSubmit({{ $manuscript->id }})" class="text-green-700 hover:text-green-900 mr-3 transition-colors duration-200 hover:underline" title="Submit Manuscript">
                                        <i class="fas fa-check-circle" style="color: #15803d;"></i> Submit
                                    </button>
                                </form>
                                @endif
                            </td>
